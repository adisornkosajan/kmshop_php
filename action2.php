	...
	require_once('PHPMailer/PHPMailerAutoload.php');
	  ...
	  if (isset($_POST['btn_insert'])) {
		...
		$status ='7';
		  ...
		  $email = $data['email'];
		  $stmt = $conn->prepare("UPDATE orders SET status ='7' WHERE id=$id");
		  $stmt->execute();
            ...
            $mail->Subject = "การขอเคลมสินค้า";  //หัวเรื่อง emal ที่ส่ง
            $mail->Body = " 
        <!DOCTYPE html>
        ...
        <body style='background-color: #fdfce9; margin: 0; padding: 0; -webkit-text-size-adjust: none; text-size-adjust: none;'>
                                                            ...
                                                            <table class='heading_block block-2' width='100%' border='0' cellpadding='0' cellspacing='0' role='presentation' style='mso-table-lspace: 0pt; mso-table-rspace: 0pt;'>
                                                                <tr>
                                                                    <td class='pad' style='padding-bottom:20px;padding-top:20px;text-align:center;width:100%;'>
                                                                        <h1 style='margin: 0; color: #066c35; direction: ltr; font-family: 'Open Sans', 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 21px; font-weight: 400; letter-spacing: 2px; line-height: 120%; text-align: center; margin-top: 0; margin-bottom: 0;'><span class='tinyMce-placeholder'>ขอเคลมสินค้า</span></h1>
                                                                    </td>
                                                                </tr>
                                                            </table>
                                                                    ...
                                                                    <td class='pad'>
                                                                        <div style='font-family: Arial, sans-serif'>
                                                                            <div class='txtTinyMce-wrapper' style='font-size: 14px; font-family: 'Open Sans', 'Helvetica Neue', Helvetica, Arial, sans-serif; mso-line-height-alt: 21px; color: #393d47; line-height: 1.5;'>
                                                                                ...
                                                                                <p style='margin: 0; font-size: 14px; text-align: left;'><p>สินค้าของคุณอยู่ในขั้นตอนการขอเคลมสินค้า กรุณาจัดส่งสินค้ามาทาง ที่อยู่ (XXX) เพื่อให้ทางเราจัดการตรวจสอบและเคลมสินค้า ขอบคุณครับ </p>
            ...
            if ($mail->Send()){
            echo "ข้อความของคุณได้ส่งเคลมแล้ว";
            }else{
            echo "การส่งไม่สำเร็จ";
			  ...
			  $filePath = 'admin/can/'.$fileNew;
			  if (in_array($fileActExt, $allow)) {
				  if ($img['size'] > 0 && $img['error'] == 0) {
					  if (move_uploaded_file($img['tmp_name'], $filePath)) {
						$stmt = $conn->prepare("INSERT INTO can_claim(status,petition,order_id,can_img) VALUES (:status,:petition,:order_id,:can_img)");
						$stmt->bindParam(":petition", $petition);
						$stmt->bindParam(":status", $status);
						$stmt->bindParam(":order_id", $id);
						$stmt->bindParam(":can_img", $fileNew);
						  $stmt->execute();
