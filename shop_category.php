  ...
  <section class="ftco-section">
   ...
   <a class="alert alert-success" style="border-radius:45px;"   href="shop.php?page=1"><font size="4">ทั้งหมด</font></a>&nbsp;&nbsp;
      ...
      <div class="row mt-2 pb-3">
        ...
        $stmt = $conn->prepare("SELECT * FROM product WHERE  category  = $id limit $strat,$limit");
        $stmt->setFetchMode(PDO::FETCH_ASSOC);
        $stmt->execute();
        $result1 = $stmt->fetchAll();

        
        $stmt22 = $conn->prepare("SELECT * FROM product WHERE  category = $id ");
                  ...
                  <div class="card-footer p-1">
                    <form action="" class="form-submit">
                      ...
                      <?php if ($result['product_qty_add'] == "0") {
                              ...
                              ?>
                                <button class="btn btn-outline-success addItemBtn" onclick='javascript:location.reload()' ><i class="buy-now d-flex justify-content-center align-items-center mx-1"  ></i>&nbsp;&nbsp;Add to cart </button>
                            <?php }
                            ?>
    ...
    <nav aria-label="Page navigation example">
      <ul class="pagination justify-content-center">
        <li class="page-item ">
          <a class="page-link" href="shop_category.php?category=<?php echo $id ?>&page=<?php echo $previous ==0 ? 1 : $previous?>" >Previous</a>
        </li>
        <?php 
        for($i=1;$i<=$total_p;$i++){
          ?>
      <li class="page-item"><a class="page-link" href="shop_category.php?category=<?php echo $id ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
          <?php
        }
        ?>

        <li class="page-item">
          <a class="page-link" href="shop_category.php?category=<?php echo $id ?>&page=<?php echo $next == $total_p ?$total_p : $next ?>">Next</a>
        </li>
      </ul>
    </nav>
    </nav>
